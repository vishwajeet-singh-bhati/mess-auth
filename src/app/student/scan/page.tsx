...
const DENIAL_MESSAGES: Record<string, { title: string; hint: string }> = {
  ...
  invalid_token:      { title: 'Invalid QR',         hint: 'Make sure you are scanning the mess QR.' },
...
export default function ScanPage() {
  ...
  // Load jsQR library on mount — works on ALL browsers including mobile
  useEffect(() => {
  ...
  // Scan loop — runs only when scanning phase is active and jsQR is loaded
  useEffect(() => {
    ...
    <div style={{
      ...
      <div style={{ position: 'absolute', inset: 0, display: phase === 'scanning' ? 'block' : 'none' }}>
        {cameraError ? (
          <div style={{
            ...
            <p style={{ color: '#fff', fontSize: '0.95rem', lineHeight: 1.6, maxWidth: '280px' }}>
              {cameraError}
            </p>
          </div>
        ) : (
      ...
      {phase === 'scanning' && !cameraError && (
        <>
          ...
          {/* Viewfinder — SQUARE, no oval */}
          <div style={{
            flex: 1, position: 'relative',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            {/* Dark overlay with square hole cut out */}
            <div style={{
              position: 'absolute', inset: 0,
              background: 'rgba(0,0,0,0.55)',
              WebkitMaskImage: 'url(#squareMask)',
            }}>
              {/* Use box-shadow trick for square cutout — no SVG needed */}
            </div>
      ...
      {phase === 'success' && result?.success && (
        <div style={{
          flex: 1, display: 'flex', flexDirection: 'column',
          background: '#0a0a0a', animation: 'fadeUp 0.35s ease both',
        }}>
          <div style={{
            ...
            <div>
              <div style={{ color: '#4ade80', fontSize: '1.75rem', fontWeight: 900, letterSpacing: '-0.03em' }}>
                Authorized
              </div>
      ...
      {phase === 'denied' && !result?.success && (
        <div style={{
          ...
          <div style={{ flex: 1, padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{
              background: '#1a1a1a', borderRadius: '12px', padding: '1.25rem',
